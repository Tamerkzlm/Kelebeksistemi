...
import { Upload, AlertCircle, ChevronRight, ChevronLeft, Search, UserX, UserCheck, Users, ImageIcon, FileSpreadsheet, Loader2, UserPlus, X, Check, Trash2 } from 'lucide-react';
...
export default function Step2Students({ students, onChange, onNext, onBack }) {
  ...
  const subeler = [...new Set(students.map(s => s.sinif))].sort((a, b) => a.localeCompare(b, 'tr'));
  const filtered = students.filter(s => {
    const mG = filterSinif === 'all' || s.sinif === filterSinif;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-5">
        ...
        <p className="text-muted-foreground text-sm">Hz. Ayşe Kız AİHL öğrencileri önceden yüklenmiştir. Ek öğrenci ekleyebilirsiniz veya eklenmiş öğrencileri silebilirsiniz.</p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm p-4 mb-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2">
            ...
            <div className="col-span-2 sm:col-span-1">
              <label className="block text-xs font-semibold text-muted-foreground mb-1">Ad Soyad (Alt alta yazılabilir) *</label>
              <textarea value={manualForm.adSoyad}
                onChange={e => setManualForm(f => ({ ...f, adSoyad: e.target.value }))}
                placeholder="Ayşe Yılmaz&#10;Ali Kaya"
                rows={2}
                className="w-full px-2.5 py-1.5 rounded-lg border border-border text-sm outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary resize-y"
              />
            </div>
            <div>
        ...
        <>
          ...
          <div className="flex gap-2 mb-3">
            ...
            >
              ...
              {subeler.map(g => <option key={g} value={g}>{g}</option>)}
            </select>
            {filterSinif !== 'all' && (
               <button onClick={() => {
                 if(window.confirm(`${filterSinif} şubesindeki tüm öğrencileri silmek istediğinize emin misiniz?`)) {
                   onChange(students.filter(s => s.sinif !== filterSinif));
                   setFilterSinif('all');
                 }
               }} className="px-3 py-2 text-sm rounded-lg border border-red-200 text-red-600 hover:bg-red-50 flex items-center gap-1 shrink-0" title="Şubeyi tamamen sil">
                 <Trash2 className="w-4 h-4" /> Şubeyi Sil
               </button>
            )}
          </div>
